...
import { robots, serializeRobots } from './robots';
...
describe('robots.txt', () => {
	...
	it('allows all crawlers in production and points them at the sitemap', () => {
		...
		expect(serializeRobots(robots())).toContain('Sitemap: https://rivus.ai/sitemap.xml');
	});

	it('disallows all crawlers in the development environment', () => {
		process.env.RIVUS_ENV = 'development';
		expect(robots().rules).toEqual({ userAgent: '*', disallow: '/' });
		expect(serializeRobots(robots())).toContain('Disallow: /');
	});
