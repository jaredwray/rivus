import type { AccountId, AgentSlot, JobId } from '@rivus/core';
...
export const agentEmailRoutes: FastifyPluginAsync = async (fastify) => {
	...
	const { config, accounts, customers, jobs, conversations, agentThreads, mailer, receivedEmails } =
		app.deps;
	...
	app.post(
		'/email/inbound',
		{
			schema: {
				tags: ['channels'],
				summary: 'Receive an inbound customer email (Resend webhook)',
				description:
					'Resend delivers every email sent to an account’s agent address ' +
					'(`<account-slug>@riv.us`) here. Rivus validates the sender against the ' +
					'account’s customers, keeps the multi-turn scheduling state on the thread, ' +
					'and replies by email — offering open times and booking the job once a ' +
					'time is agreed. Deliveries are authenticated with the Svix signature ' +
					'headers when `RESEND_WEBHOOK_SECRET` is configured.',
				body: z.unknown(),
				response: {
