...
export const agentTesterRoutes: FastifyPluginAsync = async (fastify) => {
	...
	const { accounts, agentThreads, conversations, customers } = app.deps;
	// The whole dep bag, structurally narrowed — see the note in `agent-email.ts`:
	// widening what the core reads must never mean editing a route.
