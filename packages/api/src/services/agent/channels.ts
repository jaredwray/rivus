 ...
 * used. A channel with no entry (`phone`, and `voice` until it's built) has no
 * transport — the caller records the reply without sending it. Adding a new
 * channel's entry here lights up inbox reply-out for it automatically.
 */
export function createChannelAdapters(
	...
	return {
		...
		sms: createSmsChannelAdapter({
			customers: deps.customers,
			sender: deps.smsSender,
		}),
	};
}
