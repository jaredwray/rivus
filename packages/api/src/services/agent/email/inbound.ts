...
// The Gmail/Apple Mail reply attribution — "On <date>, <sender> wrote:" — is
// matched on its own rather than as a single-line QUOTE_MARKER, because a long
// sender address makes the sending client hard-wrap the plain-text copy and
// strand "wrote:" on the next line:
//
...
export function extractReplyText(text: string): string {
	...
	for (let index = 0; index < lines.length; index += 1) {
		...
		if (startsReplyAttribution(lines, index)) {
			break;
		}
		if (/^\s*>/.test(line)) {
			continue;
